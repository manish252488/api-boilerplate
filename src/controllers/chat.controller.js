...
let questions = [];
let responses = [];
let movie_conversations = fs.readFileSync(
  path.resolve("src/data/conversation.txt")
);
...
export async function chat(req, res) {
  try {
    ...
    if (match.ratings && match.ratings.length > 0) {
      ...
      const input = {
        queries: tokens,
        responses: getSimilar.slice(randomOffset, randomOffset + numSamples),
      };
      filteredRes = filteredRes.slice(randomOffset, randomOffset + numSamples);
    ...
    } else return res.status(405).send("could not understand!");
  } catch (err) {
